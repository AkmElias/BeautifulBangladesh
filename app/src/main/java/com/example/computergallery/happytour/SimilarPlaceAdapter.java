...
/**
 * Created by Akhlak Uzzaman Ashik on 8/1/2017.
 */

public class SimilarPlaceAdapter extends RecyclerView.Adapter<SimilarPlaceAdapter.MyViewHolder> {

    private ArrayList<SimilarPlaceModel> similarPlaceModels;
    private Context context;
    private SQLiteHandler db;
    private SQLiteHandlerForOnePlace dbb;
    private SessionManager session;
    String place_id;
    SimilarPlaceModel newsModel;
    SliderLayout sliderShow;
    private static final String TAGG = AllPlaceAdapter.class.getSimpleName();

    public SimilarPlaceAdapter(Context context, List<SimilarPlaceModel> postLists) {
        this.similarPlaceModels = (ArrayList<SimilarPlaceModel>) postLists;
        this.context = context;
    }


    @Override
    public SimilarPlaceAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(context).inflate(R.layout.all_place_card_view, parent, false);
        SimilarPlaceAdapter.MyViewHolder holder = new SimilarPlaceAdapter.MyViewHolder(view, context, similarPlaceModels);
        return holder;
    }

    @Override
    public void onBindViewHolder(final SimilarPlaceAdapter.MyViewHolder holder, int position) {
        final SimilarPlaceModel newsModel = similarPlaceModels.get(position);
...
//        Double d=Double.valueOf(twoDecimals.format(value));
        holder.rating.setText("Rating :"+newsModel.getRating());
...
//                .into(holder.imageView);
    ...
    public int getItemCount() {
        return similarPlaceModels.size();
    }

    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        TextView name,rating,fact;
        ImageView imageView;
        ArrayList<SimilarPlaceModel> postLists = new ArrayList<>();
        ProgressWheel progressWheel;
        Context context;

        public MyViewHolder(View itemView, Context context, ArrayList<SimilarPlaceModel> postLists) {
            super(itemView);
            this.context = context;
