...
public class allHotelAdapter extends RecyclerView.Adapter<allHotelAdapter.MyViewHolder> {
    ...
    private ArrayList<allHotelModel> cultureModels;
    private Context context;
    private SQLiteHandler db;
    private SQLiteHandlerForOnePlace dbb;
    private SessionManager session;
    String place_id;
    allHotelModel newsModel;
    SliderLayout sliderShow;
    private static final String TAGG = AllPlaceAdapter.class.getSimpleName();

    public allHotelAdapter(Context context, List<allHotelModel> postLists) {
        this.cultureModels = (ArrayList<allHotelModel>) postLists;
        this.context = context;
    }


    @Override
    public allHotelAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(context).inflate(R.layout.all_place_card_view, parent, false);
        allHotelAdapter.MyViewHolder holder = new allHotelAdapter.MyViewHolder(view, context, cultureModels);
        return holder;
    }

    @Override
    public void onBindViewHolder(final allHotelAdapter.MyViewHolder holder, int position) {
        final allHotelModel newsModel = cultureModels.get(position);
...
//                .into(holder.imageView);
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        ImageView imageView;
        ArrayList<allHotelModel> postLists = new ArrayList<>();
        ProgressWheel progressWheel;
        Context context;

        public MyViewHolder(View itemView, Context context, ArrayList<allHotelModel> postLists) {
            super(itemView);
            this.context = context;
